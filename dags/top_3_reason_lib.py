...
# ----------------------------------------------------------
SOURCE_TABLE_1 = "gbm1_predictions_jfmamj_final_segment_reasons"
SOURCE_TABLE_2 = "historical_data_with_not_renewed_reason_only"

TARGET_TABLE = "gbm1_predictions_jfmamj_final_top3_reasons"
...
def top_3_reason():
    ...
    # Preprocess
    df_hist['not_renewed_reasons'] = df_hist['not_renewed_reasons'].fillna("").astype(str)

    # Step 2: Explode historical reason counts
    all_reasons = df_hist['not_renewed_reasons'].str.split(',').explode().str.strip()
    reason_counts = all_reasons.value_counts().reset_index()
    reason_counts.columns = ['Reason', 'Count']
    print(f"exploded historical reason count")
    ...
    print(f"ranking historical reason")
    ...
    df['not_renewed_reasons'] = df['not_renewed_reasons'].fillna("").astype(str)
    ...
    # ---------------------------
    df['Top 3 Reasons'] = df['not_renewed_reasons'].apply(get_top_3_reasons)
