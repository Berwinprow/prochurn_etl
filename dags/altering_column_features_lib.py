...
TABLE = "overall_cleaned_base_and_pr_ef_policyef"
...
def step_log(label, func):
    ...
    print(f"✅ COMPLETED: {label} in {int((end-start)//60)} min {int((end-start)%60)} sec")
    print(f"-----------------------------\n")
...
def convert_and_reload():
    ...
    def clean_new_vertical():
        ...
        print(f"   Sample after:  {df['cleaned_new_vertical'].iloc[0]}")
    ...
    def clean_previous_ncb():
        before_blank = (df["previous_year_ncb_percentage"] == "(blank)").sum()
    ...
    def clean_discount_ncb():
        before_blank = (df["applicable_discount_with_ncb"] == "(blank)").sum()
    ...
    def clean_ncb_amount():
        ...
        df["ncb_amount"] = df["ncb_amount"].replace("(blank)", None)
        df["ncb_amount"] = pd.to_numeric(df["ncb_amount"], errors="coerce")
    ...
    def clean_before_gst():
        before_blank = (df["before_gst_add_on_gwp"] == "(blank)").sum()

        df["before_gst_add_on_gwp"] = df["before_gst_add_on_gwp"].replace("(blank)", None)
        df["before_gst_add_on_gwp"] = pd.to_numeric(df["before_gst_add_on_gwp"], errors="coerce")
        df["before_gst_add_on_gwp"] = df["before_gst_add_on_gwp"].round().astype(float)
