...
def model_file_generation():
    ...
    # Filter the main dataset for customers whose policy_end_date is <= July 2024
    data = data[data['policy_status'].isin(['Renewed', 'Not Renewed'])]

    # Map policy_status to binary for the filtered dataset
    data['policy_status'] = data['policy_status'].apply(lambda x: 1 if x == 'Not Renewed' else 0)
