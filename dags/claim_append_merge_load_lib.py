...
# ---------------------------------------------------------------------
# List of claim tables to append (in correct ascending order)
# ---------------------------------------------------------------------
CLAIM_TABLES = [
    "claim_2022_part_1",
    "claim_2022_part_2",
...
def append_claim_table():
    ...
    # 2️⃣ Extract each claim table
    for table_name in claim_tables:

        print(f"📥 Loading table: {table_name}")
        df = pd.read_sql(f'SELECT * FROM pip_stage."{table_name}"', engine)

        # Normalize column names for safe merge
        df.columns = (
            df.columns
            .str.strip()
            .str.lower()
    ...
    print(f"✅ Successfully created {TARGET_SCHEMA}.claim_append with {len(final_df)} rows.")
...
def merge_claim_table():
    ...
    # Before removing duplicates, count the occurrences of each status in 'updated_status'
    ...
    # Step 8: Merge the status counts from 'updated_status' and 'Claim status' back into the filtered DataFrame to retain all columns
...
def merge_basepr_with_claim():
    ...
    with engine.begin() as conn:
        conn.execute(text(f"DROP TABLE IF EXISTS {TARGET_SCHEMA}.{FINAL_TABLE}"))
        print(f"{FINAL_TABLE} table dropped before inserting new datas")
