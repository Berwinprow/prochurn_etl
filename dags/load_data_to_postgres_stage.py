...
def batch_process_from_blob(**context):
    ...
    # --- 1️⃣ Fetch Azure connection and pipeline config ---
    ...
    # --- 2️⃣ Get optional file filters from Airflow trigger or Variables ---
    dag_run_conf = context.get("dag_run").conf or {}

    # e.g. Trigger with {"selected_files": ["base_2023.xlsx", "pr_2023.xlsx"]}
    selected_files = dag_run_conf.get("selected_files")

    # If no file list is passed, fall back to Variables or default (process all)
    ...
    logging.info(f"[batch_process_from_blob] Selected files: {selected_files if selected_files else 'All files'}")

    # --- 3️⃣ Initialize Azure Blob service ---
    blob_service_client = BlobServiceClient.from_connection_string(AZURE_STORAGE_CONNECTION_STRING)
    container_client = blob_service_client.get_container_client(AZURE_BLOB_CONTAINER)

    # --- 4️⃣ List and filter blobs based on selection ---
    all_blobs = [blob for blob in container_client.list_blobs() if blob.name.lower().endswith((".csv", ".xlsx", ".xlsb"))]

    if selected_files:
        blob_list = [blob for blob in all_blobs if blob.name in selected_files]
    else:
        blob_list = all_blobs  # fallback — process everything

    total_files = len(blob_list)
    logging.info(f"[batch_process_from_blob] Found {total_files} files to process in Azure Blob Storage.")

    if total_files == 0:
        raise Exception(f"No matching files found for selection: {selected_files}")

    # --- 5️⃣ Loop through selected files and process ---
    errors = []
    ...
    for i, blob in enumerate(blob_list, start=1):
        ...
        try:
            ...
            logging.info(f"[batch_process_from_blob] Processing file {i}/{total_files}: {file_name}")

            process_file_bytes_to_postgres(
                file_bytes, file_name, engine, SCHEMA_NAME, fernet=None, columns_to_encrypt=None
            )

            logging.info(f"[batch_process_from_blob] ✅ Successfully processed file {i}/{total_files}: {file_name}")

        except Exception as e:
            logging.error(f"[batch_process_from_blob] ❌ Error during processing of {file_name}: {e}", exc_info=True)
            errors.append(f"{file_name}: {str(e)}")

    # --- 6️⃣ Handle failed files ---
    if errors:
        raise Exception(f"Some files failed to load: {errors}")

    logging.info("[batch_process_from_blob] ✅ All selected files processed successfully.")
