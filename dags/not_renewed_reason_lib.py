...
# ----------------------------------------------------------
SOURCE_TABLE_1 = "gbm1_prediction_jfmamj_final"
SOURCE_TABLE_2 = "policydata_with_fb_cc_pc_newfea_opti_correct"

TARGET_TABLE_1 = "gbm1_predictions_jfmamj_final_segment_reasons"
TARGET_TABLE_2 = "historical_data_with_not_renewed_reason_only"
...
def reason_for_churn(row,status_col):
    if row[status_col] == 'Not Renewed':
        ...
        # tie_ups (grouped)
        if row['tie_up'] in ['MARUTI', 'MIBL OEM', 'HYUNDAI', 'Non-OEM']:
...
def call_pred_data_def():
    ...
    query = f"SELECT * FROM {SOURCE_SCHEMA}.{SOURCE_TABLE_1} WHERE policy_status = 'Not Renewed';"
    df = pd.read_sql(query, con=engine)
    print(f"✅ Loaded {len(df)} rows from {SOURCE_TABLE_1}")

    # Apply the function
    df['Not Renewed Reasons'] = df.apply(lambda r: reason_for_churn(r,'predicted_status'), axis=1)
    print("functions applied sucessfully")
    ...
    print(f"loaded data sucessfully into db with total records of: {len(df)} in {TARGET_TABLE_1}")
...
def call_historic_data_def():
    ...
    # Step 1: Load Data
    # ---------------------------
    query = f"SELECT * FROM {SOURCE_SCHEMA}.{SOURCE_TABLE_2};"
    df = pd.read_sql(query, con=engine)
    ...
    numeric_columns = [
        'before_gst_add_on_gwp', 'total_od_premium', 'total_tp_premium', 'total premium payable', 
        'previous_year_ncb_percentage', 'applicable_discount_with_ncb', 'vehicle_age', 'vehicle_idv'
    ]
    ...
    # Apply the function
    df['Not Renewed Reasons'] = df.apply(lambda r: reason_for_churn(r,'policy_status'), axis=1)
    ...
    print(f"loaded data sucessfully into db with total records of: {len(df)} in {TARGET_TABLE_2}")
