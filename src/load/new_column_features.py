...
from utils.schema_table_config import get_column_mapping, get_log_tables, get_schema
...
TARGET_TABLE = "final_policy_features_encrypt"
# TABLE = "baseprclaim"
...
def get_latest_addons_table(engine):
    ...
    if result:
        ...
        logger.warning(f"no table found in {CLAIM_TABLE}")
        return None
...
def build_policy_features():
    try:
        ...
        if not SOURCE_TABLE:
            logger.warning(f"no source table found in {SOURCE_TABLE}")
            return
        logger.info(f"found table to process {SOURCE_TABLE}")
        ...
        logger.info(f"datas loaded to query")
        ...
        for col in df.columns:
            if col in sensitive_cols:
                logger.info(f"🔓 Decrypting column {col} in batches")

                for i in range(0, len(df), BATCH):
                    df.loc[i:i+BATCH, col] = (
                        df.loc[i:i+BATCH, col]
                        .map(lambda v: (
                            v if pd.isna(v) or not isinstance(v, str)
                            else (
                                fernet.decrypt(v.encode()).decode()
                                if v.startswith("gAAAA")  # Fernet tokens always start like this
                                else v
                            )
                        ))
                    )

                    logger.info(f"   decrypted rows {i} → {min(i+BATCH, len(df))}")

        gc.enable()
        logger.info(f"decrypt done")
        ...
        # Previous rounded premium (FAST)
        df["prev_premium"] = (
            df.groupby(group_cols)["curr_premium"].shift()
        )
        ...
        def renewal_status_logic(row):
            ...
            if row["curr_premium"] > row["prev_premium"]:
                ...
                return "No Change"
        logger.info(f"main funct started")
        # default
        df["renewal_rate_status"] = "No Change"

        # CASE 1: no previous policy
        mask_no_prev = df["prev_policy_end_date"].isna()

        # CASE 2: overlap / continuous
        mask_overlap = (
            df["policy_start_date"]
            < (df["prev_policy_end_date"] + pd.Timedelta(days=1))
        )

        # CASE 3: premium comparison
        mask_increase = df["curr_premium"] > df["prev_premium"]
        mask_decrease = df["curr_premium"] < df["prev_premium"]

        # apply rules (same order as SQL CASE)
        df.loc[mask_no_prev | mask_overlap, "renewal_rate_status"] = "Null"
        df.loc[~mask_no_prev & ~mask_overlap & mask_increase, "renewal_rate_status"] = "Increase"
        df.loc[~mask_no_prev & ~mask_overlap & mask_decrease, "renewal_rate_status"] = "Decrease"
        ...
        logger.info(f"renewal status done")
        # =====================================================================
        ...
        if 'renewed_flag' in df.columns:
            ...
            logger.info(df['policy_status'].value_counts(dropna=False))
        logger.info(f"renwed flag done")
        ...
        if {'customer_id', 'end_year', 'policy_status'}.issubset(df.columns):
            ...
            logger.info(f"chrun label done")
        ...
        # ========== END: New Columns (Inserted Mid-Script) ==========
            logger.info("new column adding done")
        ...
        logger.info("days gap prev end to curr start done")
        ...
        logger.info(f" Feature engineering complete – rows: {len(df):,}, cols: {df.shape[1]}")
        ...
        logger.info("policy wise purchase column")
        ...
        logger.info(f"df[['initial_policy_no', 'policy_wise_purchase']].head()")

        logger.info("pricing catelog column adding script")
        ...
        logger.info(f"✅ Pricing catalog columns added: {[c for c in pricing_catalog.columns if c not in pricing_grp_col]}")

        try:
            logger.info("🧹 Disposing old engine before final write...")
            engine.dispose()
            time.sleep(2)
            ...
            logger.info("🚀 Writing final data to Postgres...")
            ...
            with retry_engine.begin() as conn:
                ...
                for i in range(0, len(df), step):
                    ...
                    logger.info(f"✅ wrote rows {i}–{i+step}")
            logger.info("✅ Retry successful after rollback.")
        finally:
            fresh_engine.dispose()
            gc.collect()
            logger.info("🧹 Engine disposed and memory cleaned up.")
    except Exception:
        logger.error(
