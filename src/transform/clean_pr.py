...
def archive_and_log_removed_rows(
    ...
    if removed_df.empty:
        logger.info("No removed rows to log for %s", source_table)
        return
        ...
        if exists:
            ...
            conn.execute(
                text(f"""
                    ...
                    AS SELECT * FROM {log_schema}.{log_table}
                """)
            )
            conn.execute(
                text(f"DROP TABLE {log_schema}.{log_table}")
            )
...
def load_pr_table():
    ...
    with engine.begin() as conn:
        ...
        if results:
            ...
            logger.info(f"⚠️ No PR tables found in {SOURCE_SCHEMA}, skipping base_clean and moving on.")
            return []
...
def clean_and_load_pr_data():
    ...
    logger.info(f"🛠 Starting Data Processing at `{pipeline_run_time}`")
    source_tables = load_pr_table()
    if not source_tables:
        logger.info("🚫 No PR tables pending cleaning.")
        return
    ...
    for source_table in source_tables:
        ...
        try:
            ...
            logger.info(f"📂 Extracted {initial_count} records from `{SOURCE_SCHEMA}.{source_table}`.")
            ...
            logger.info(f"🔄 Column Mapping Applied: {len(mapped_columns)} columns changed.")
            ...
            logger.info("🧼 Cleaned chassis, engine, insured name, vehicle reg no, and branch name.")
            # ---------------------------------------------------------
            ...
            if "zone" in df.columns:
                ...
                logger.info("✅ PR 2023 → zone cleaned (NORTH/SOUTH/EAST/WEST, Corporate → NULL)")
            else:
                df["zone"] = None
                logger.info("✅ PR 2023 → zone column missing, added as NULL")
            ...
            if "cleaned_chassis_no" in df.columns and "cleaned_engine_no" in df.columns:
                ...
                logger.info("🔗 Concatenated chassis_no & engine_no into `concat_chassis_engine`.")


            # ✅ Step 3: Clean & Convert `policy_number`
            removed_invalid_policy = df[df[POLICY_NUMBER_COLUMN].apply(lambda x: clean_policy_number(x) is None)].copy()
            ...
            logger.info(f"🔍 Removed {len(removed_invalid_policy)} invalid policy numbers.")
            ...
            if NOP_COLUMN in df.columns:
                ...
                logger.info(f"📌 Removed {len(removed_nop)} rows where `nop` != 1.")
            else:
                removed_nop = pd.DataFrame()  # Create an empty DataFrame if column doesn't exist
                logger.info("⚠️ `nop` column not found, skipping `nop` filtering step.")
            ...
            logger.info(f"💰 Removed {len(removed_premium_issues)} rows with invalid `total_premium`.")
            ...
            logger.info(f"📉 Removed {len(removed_short_policies)} policies with duration <= 10 months.")
            ...
            before_dedup = len(df)
            df_sorted = df.sort_values(POLICY_ISSUE_COLUMN, ascending=False)
            removed_duplicate_policies = df_sorted[
                df_sorted.duplicated(subset=["policy_key"], keep="first")
            ].copy()

            df = df_sorted.drop_duplicates(
                subset=["policy_key"], keep="first"
            )

            removed_duplicate_policies["removal_reason"] = (
                "Duplicate policy, keeping latest issue date"
            )

            logger.info(f"📊 Removed {len(removed_duplicate_policies)} duplicate policies.")
            ...
            if NET_PREMIUM_COLUMN in df.columns:
                ...
                logger.info(f"💰 Calculated gst and total_premium_payable.")

            # ✅ Final count
            final_count = len(df)
            logger.info(f"✅ Final record count after cleansing: {final_count} (Removed {initial_count - final_count} total rows).")
            ...
            if not removed_data.empty:
                removed_data["source_table"] = source_table
            ...
            df.to_sql(name=target_table, schema=TARGET_SCHEMA, con=engine, if_exists="append", index=False, dtype={POLICY_NUMBER_COLUMN: String})
            row_count = len(df)

            logger.info(f"✅ Data successfully loaded into `{TARGET_SCHEMA}.{target_table}` with {row_count} rows.")
            clean_cnt = len(df)
            removed_cnt = len(removed_data)
            ...
            logger.info(f"data successfully updated to {META_TABLE}")
        except Exception:
            logger.error(
                "❌ Failed to process PR table %s",
                source_table,
                exc_info=True
            )
            raise
