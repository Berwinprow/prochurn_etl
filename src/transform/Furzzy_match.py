...
logger.info(Variable.get("renewal_ref_date"))
import gc
import time
...
def get_latest_source_table(engine):
    ...
    if result:
        ...
        logger.warning(f"no table found in {FEATURE_LOG}")
        return None
...
def fuzzy_matching():
    try:
        ...
        if not SOURCE_TABLE:
            logger.warning("no source table found")
            return
        ...
        logger.info(f"📂 Loaded {len(df)} records from `{SOURCE_SCHEMA}.{SOURCE_TABLE}`.")
        ...
        logger.info("✅ Customer ID & Name successfully propagated for duplicate policies!")
        ...
        logger.info("✅ Customer ID successfully propagated!")
        ...
        logger.info(f"fetched date from Airflow UI : {ref_date}")
        ...
        logger.info("✅ Policy renewal identification completed.")
        ...
        logger.info("✅ customer_tenure & renewal status calculated.")
        ...
        try:
            ...
            logger.info(f"✅ Data successfully loaded into `{TARGET_SCHEMA}.{TARGET_TABLE}`.")
            row_cnt = len(df)

            update_claim_merge_table(fresh_engine, TARGET_TABLE, row_count=row_cnt)
            logger.info("✅ Metadata updated with claim merge table name.")

        except (PendingRollbackError, OperationalError) as e:
            logger.warning(f"❌ Transaction issue encountered: {e}. Retrying after rollback...")
        ...
        logger.error(
            "❌ Failed during fuzzy matching pipeline",
            exc_info=True
        )
        raise
